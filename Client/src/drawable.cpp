...
void drawLobbyPlayer(Extends extends, std::string name, bool ready, bool you) {
    ...
    OpenGL::drawImage(path, ext);
    if(ready) OpenGL::drawText("READY", alignExtends(extends,0,-0.04f,1,0.15f), COLOR_GREY, TEXTSIZE_MEDIUMSMALL);
    OpenGL::drawText(name, alignExtends(extends,0,0.04f,1,0.15f), COLOR_BLACK, TEXTSIZE_MEDIUM);
}
