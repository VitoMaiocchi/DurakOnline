...
- Specialized Child Nodes:
  ...
  - back_button: Button to return to the previous (login)screen.
  - ready_button: Button to toggle readiness.
  - settings_button: Button to access settings.
  - player_nodes: List of nodes representing players, used for drawing (name, if ready, etc etc).
...
- Methods:
  ...
  - layerUpdateNotify(): Notifies the lobby of changes in player data.
